...
class CyberEnv(BaseEnv):
    ...
    def reinitialize_midsize_network(self):
        ...
        edges = [(self.senders[0],self.routers[0]), (self.senders[1],self.routers[1]), (self.routers[0],self.routers[1]), 
        (self.routers[0],self.routers[2]),(self.routers[0],self.routers[3]), (self.routers[1],self.routers[2]),(self.routers[1],self.routers[3]), (self.routers[2],self.routers[4]), (self.routers[3],self.routers[4]),(self.routers[4],self.interpreter)]
    ...
    def reset(self):
        ...
        compromised_rtr = []
        if self.with_threat:
            if self.deviceCount == 5:
                compromised_rtr = self.threat_model_midsize_case(random.randint(0,1))
            else:
                compromised_rtr =self.threat_model(random.randint(0,1))
