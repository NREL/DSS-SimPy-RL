...
class CyberEnv(BaseEnv):
    ...
    def __init__(self, provided_graph= None,channelModel=True, envDebug=True, R2_qlimit = 100, ch_bw=1000, with_threat= False, comp_zones = None):
        super(CyberEnv, self).__init__(deviceCount=6)
        ...
        if provided_graph is None:
            ...
            self.action_space = spaces.MultiDiscrete([self.deviceCount, 3])
    ...
    def reinitialize_midsize_network(self):
        ...
        self.routers: List[self.ForwarderDevice] = [
            ...
            CyberEnv.ForwarderDevice(SimMan.env,'R6',rate=200.0, qlimit=200,debug=self.envDebug)
        ]
        ...
        self.channels : List[self.channels] = [
            ...
            CyberEnv.Channel(SimMan.env, 'C46',src = self.routers[3],dest=self.routers[5],bw=self.ch_bw),
            CyberEnv.Channel(SimMan.env, 'C56',src = self.routers[4],dest=self.routers[5],bw=self.ch_bw),
            CyberEnv.Channel(SimMan.env, 'CS',src = self.routers[5],dest=self.interpreter,bw=self.ch_bw)
        ]
        self.edges.extend(self.channels)
        ...
        self.routers[3].out.append(self.routers[5])
        self.routers[4].out.append(self.routers[5])
        self.routers[5].out.append(self.interpreter)
        ...
        self.routers[2].out_channel.append(self.channels[6])
        self.routers[3].out_channel.append(self.channels[7])
        self.routers[3].out_channel.append(self.channels[8])
        self.routers[4].out_channel.append(self.channels[9])
        self.routers[5].out_channel.append(self.channels[10])

        edges = [(self.senders[0],self.routers[0]), (self.senders[1],self.routers[1]), (self.routers[0],self.routers[1]), 
        (self.routers[0],self.routers[2]), (self.routers[1],self.routers[2]),(self.routers[1],self.routers[3]), (self.routers[2],self.routers[4]), 
        (self.routers[3],self.routers[4]),(self.routers[3],self.routers[5]),(self.routers[4],self.routers[5]),(self.routers[5],self.interpreter)]
    ...
    def reset(self):
        ...
        if self.with_threat:
            if self.deviceCount == 6:
                compromised_rtr = self.threat_model_midsize_case(random.randint(1,1))
            else:
                compromised_rtr = self.threat_model(random.randint(1,1))
        ...
        self.rtr_compromised = compromised_rtr
