...
for c in range(1000,2600,500):
    ...
    for r in range(80,130,20):
        ...
        random_epi_len,rnd_succ_epi_len,_ = expert_rollouts(env,episodes=50,rtrs_comp=['R3','R4','R5'],_expert=False)
        results['avg_episode_length_random']= random_epi_len
        results['succ_avg_episode_length_random']= rnd_succ_epi_len
        #time.sleep(5)

        expert_epi_len,exp_succ_epi_len,rollouts = expert_rollouts(env,episodes=400,rtrs_comp=['R3','R4','R5'])
        ...
        scipy.io.savemat('GAIL_Actual_Expert_CM_2_medium_training_qlimit_'+str(r)+'_ch_BW_'+str(c)+'goal_5pkt_each_single_threat.mat',results)
        discrim ='case_gail_CM_2_reward_qlimit_medium_training_'+str(r)+'_ch_bw_'+str(c)+'after_training_single_threat.pt'
        generat='case_gail_CM_2_policy_qlimit_medium_training_'+str(r)+'_ch_bw_'+str(c)+'after_training_single_threat.pt'
