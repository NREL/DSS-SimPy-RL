# -*- coding: utf-8 -*-
"""
This is the code for training and testing the AIRL agent on the cyber-physical co-simulation RL environment.  
"""
import sys
...
def train_and_evaluate(comp_zones, exp_tajectories, channel_bws, router_qlimits, policy_net_train_len,airl_train_len):
    """For different combination of channel bandwidths, router queue limits and expert demonstrations, train and test the policy and saves the results, reward and policy network.
        ...
        :param exp_tajectories: List of the number of expert demonstrations steps considered for AIRL training
    ...
    for exp_traj in exp_tajectories:
        for c in channel_bws:
            ...
            for r in router_qlimits:
                ...
                episode_len_after_training,succ_epi_len_after_training,learner_rewards_after_training = evaluate_policy(cyber_phy_env,airl_trainer.gen_algo)
                ...
                scipy.io.savemat('CPS_AIRL_trlen_'+str(airl_train_len)+'_exp_traj_'+str(exp_traj)+'_qlimit_'+str(r)+'_ch_BW_'+str(c)+'goal__random_rtr_one_threat.mat',results)
                discrim ='airl_trlen_'+str(airl_train_len)+'_exp_traj_'+str(exp_traj)+'_reward_qlimit_'+str(r)+'_ch_bw_'+str(c)+'after_training_one_threat.pt'
                generat='airl_trlen_'+str(airl_train_len)+'_exp_traj_'+str(exp_traj)+'_policy_qlimit_'+str(r)+'_ch_bw_'+str(c)+'after_training_one_threat.pt'
...
if __name__ == "__main__":
    ...
    train_and_evaluate(comp_zones, exp_tajectories,channel_bws, router_qlimits,policy_network_train_len,airl_train_len)