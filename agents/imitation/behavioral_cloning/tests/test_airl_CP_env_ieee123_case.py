...
from envs.simpy_dss.SimChannelDssEnvUpdated import CyberPhysicalEnvMT,CyberPhysicalMapping
import torch
# bigger case
#G = create_network()
...
for c in range(1500,1600,500):
    print('Channel BW : {0}'.format(c))
    for r in range(120,130,20):
        results = {}
        G = create_network2()
        cenv = CyberEnv(provided_graph=G, channelModel=True,envDebug=False, R2_qlimit=r, ch_bw = c,with_threat=True,comp_zones=comp_zones)
        ...
        airl_trainer.train(3000)  # Note: set to 300000 for better results
        ...
        scipy.io.savemat('CPS_AIRL_qlimit_'+str(r)+'_ch_BW_'+str(c)+'goal__random_rtr_weak_threat.mat',results)
        discrim ='reward_qlimit_'+str(r)+'_ch_bw_'+str(c)+'after_training_weak_threat.pt'
        generat='policy_qlimit_'+str(r)+'_ch_bw_'+str(c)+'after_training_weak_threat.pt'
