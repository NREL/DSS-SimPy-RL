...
for c in range(2500,2600,500):
    ...
    for r in range(80,130,20):
        ...
        rollouts = rollout.rollout(
            expert,
            DummyVecEnv([lambda: RolloutInfoWrapper(CyberEnv(provided_graph=G, channelModel=False,envDebug=False, R2_qlimit=r, ch_bw = c,with_threat=True))]),
        ...
        venv = DummyVecEnv([lambda: CyberEnv(provided_graph=G,channelModel=True,envDebug=False, R2_qlimit=r, ch_bw = c,with_threat=True)])
        ...
        scipy.io.savemat('AIRL_qlimit_'+str(r)+'_ch_BW_'+str(c)+'goal_70_pkt_const_pkt_size_50_random_rtr_strong_threat.mat',results)
        discrim ='reward_qlimit_'+str(r)+'_ch_bw_'+str(c)+'after_training_strong_threat.pt'
        generat='policy_qlimit_'+str(r)+'_ch_bw_'+str(c)+'after_training_strong_threat.pt'
