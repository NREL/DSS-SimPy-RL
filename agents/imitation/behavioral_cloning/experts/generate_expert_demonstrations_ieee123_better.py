            ...
            if len(rtr_comp) > 0 and expert:
                ...
                for parent_router in parent_routers:
                    ...
                    for ix,item in enumerate(parent_router.out):
                        ...
                        # here check if the child node is on the shortest path to the PS
                        #print('from node {0}'.format(int(name[1:])))
                        path_to_receiver =nx.single_source_shortest_path(env.G, int(name[1:]))['PS']
                        #paths_to_receiver = nx.all_simple_paths(env.G, source=int(name[1:]), target='PS')
                        #print(len(path_to_receiver))
                        #print('********************')
                        found_safe_path = True
                        
                        for k in path_to_receiver:
                            if 'R'+str(k) in rtr_comp[:2]:
                                found_safe_path = False
                                break
                        
                        if len(path_to_receiver) > 0 and found_safe_path:
                            pdr_child_nodes_ix.append(int(name[1:]))
                    ...
                    if comp_rtr_obj.packets_drop > 5:
